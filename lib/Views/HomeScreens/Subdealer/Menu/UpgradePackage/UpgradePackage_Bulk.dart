...
import 'package:sales_app/Views/HomeScreens/Subdealer/Menu/UpgradePackage/ActiveAndEligiblePackages_Bulk.dart';
...
class UpdatePackage_Bulk extends StatefulWidget {
  bool enableMsisdn;
  String  preMSISDN;
  ...
  UpdatePackage_Bulk(this.Permessions,this.role,this.outDoorUserName,this.bulkNumbers);

  @override
  _UpdatePackage_BulkState createState() => _UpdatePackage_BulkState(this.Permessions,this.role,this.outDoorUserName,this.bulkNumbers);
}

class _UpdatePackage_BulkState extends State<UpdatePackage_Bulk> {
  bool enableMsisdn;
  ...
  _UpdatePackage_BulkState(this.Permessions,this.role,this.outDoorUserName,this.bulkNumbers);
  ...
  void initState() {
    ...
    for (var i = 0; i <= bulkNumbers.length; i++) {
      if(i==0 && bulkNumbers.length==1){
        setState(() {
          MSISDN_ONE=bulkNumbers[0]['msisdn'];
          print(MSISDN_ONE);

        });
      }
      if(i==1 && bulkNumbers.length==2){
        setState(() {
          MSISDN_TWO=bulkNumbers[1]['msisdn'];
          print(MSISDN_TWO);
        });
      }
      if(i==2 && bulkNumbers.length==3){
        setState(() {
          MSISDN_THREE=bulkNumbers[2]['msisdn'];
          print(MSISDN_THREE);
        });
      }
    }
  ...
  CheckMSISDN() async {
    ...
    var url = urls.BASE_URL+'/Tawasol/sdlr/is-assigned/${bulkNumbers[0]['msisdn']}';
    print(url);
    ...
    if(statusCode==200 ){
      ...
      if(result['status']==0){
        ...
        showToast(result['message'],
            ...
            fullWidth: true):  showToast(result['messageAr'],
            context: context,
            animation: StyledToastAnimation.scale,
            fullWidth: true);
       // SendOtp();
  ...
  getCurrentPackege () async{
    ...
    var url = urls.BASE_URL+'/ChangePackage/current/${bulkNumbers[0]['msisdn']}/${otp}';
    final response = await http.get(Uri.parse(url),headers: {
      "content-type": "application/json",
      "Authorization": prefs.getString("accessToken")
    });
    ...
    if(statusCode==200 ){
      ...
      if (result["status"] == 0 && result["data"]['packageID']!='0') {
        ...
        Navigator.of(context).push(MaterialPageRoute(
            builder:(context)=>ActiveAndEligiblePackages_Bulk(
                EasyLocalization.of(context).locale == Locale("en", "US")
                    ? result["data"]["englishDescription"]
                    : result["data"]["englishDescription"],
                bulkNumbers,enableMsisdn,this.Permessions,this.role,this.outDoorUserName)));
        setState(() {
          isLoaading=false;
        ...
        Navigator.of(context).push(MaterialPageRoute(
            builder:(context)=>ActiveAndEligiblePackages_Bulk(
                EasyLocalization.of(context).locale == Locale("en", "US")
                    ...
                    : 'لا يوجد حزم نشطة',bulkNumbers,enableMsisdn,this.Permessions,this.role,this.outDoorUserName)));
        setState(() {
          isLoaading=false;
        });
      }
    ...
    return GestureDetector(
      ...
      child: Scaffold(
        ...
        body:Container(
          ...
          TextButton(
            child: Text(
              "Menu_Form.MSSID_Numbers".tr().toString(),
              style: TextStyle(
                  color: Colors.black,
                  letterSpacing: 0,
                  fontSize: 21,
              ...
              TextButton(
                child: Text(
                  "Menu_Form.Decleration_Bulk".tr().toString(),
          ...
          floatingActionButton: ListView(
            ...
            children: [
              ...
              SingleChildScrollView(
                child: Column(
                  children: [
                    isLoaading==true?Container():   Container(
                      height: 48,
                      width: 420,
