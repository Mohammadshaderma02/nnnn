...
import 'EditPendingContractDetails.dart';
class PendingContracts extends StatefulWidget {
  var Permessions=[];
  var role;
  var outDoorUserName;
  PendingContracts({this.Permessions,this.role,this.outDoorUserName});
  @override
   _PendingContractsState createState() => _PendingContractsState(this.Permessions,this.role,this.outDoorUserName);
}

class _PendingContractsState extends State<PendingContracts> {
  ScrollController scrollController = ScrollController();
  GetPendingLineDocQueueBloc getPendingLineDocQueueBloc;
  var Permessions=[];
  var role;
  var outDoorUserName;
  _PendingContractsState(this.Permessions,this.role,this.outDoorUserName);
...
void callApi (int page) async{
  ...
  var url =urls.BASE_URL+ '/LineDocumentation/pending?Limit=15&Page=${page}&Status=0';
  SharedPreferences prefs = await SharedPreferences.getInstance();
  ...
  Widget build(BuildContext context) {
      return Scaffold(
        appBar: AppBar(
          ...
          centerTitle:false,
          title: Text("DashBoard_Form.pending_contracts".tr().toString()),
                              ...
                              SizedBox(
                                ...
                                child:  ListTile(
                                  ...
                                  trailing: GestureDetector(
                                    onTap : UserList[index]['isEditable']==false ?null : Permessions.contains('01.01.04')==true?
                                      () => Navigator.of(context).push(MaterialPageRoute(builder:(context)=>EditContractDetails(data:UserList[index],Permessions:Permessions,role:role,outDoorUserName:outDoorUserName))):null,
                                    child: new Text(
                                      "DashBoard_Form.edit".tr().toString(),
                                      textAlign: TextAlign.center,
                                      style: TextStyle(
                                          color:  UserList[index]['isEditable']? Permessions.contains('01.01.04')==true?
                                          Color(0xff0070c9):
                                          // permissio denied
                                          Color(0xFFA4B0C1):
                                          //not editibale
