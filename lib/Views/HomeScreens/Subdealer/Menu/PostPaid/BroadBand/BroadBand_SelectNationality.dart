...
class _BroadBandNationalityListState extends State<BroadBandNationalityList> {
  ...
  // BroadBand-specific state variables
  bool isRental = true;
  ...
  void initState() {
    ...
    // ✅ CRITICAL: Reset all document data when screen is loaded
    _resetAllDocumentData();
    
    // ✅ CRITICAL: Initialize eKYC session (generates token and session)
    generateEkycToken();
    
    // ✅ CRITICAL: Initialize deep link listener for Sanad callback
...
// Info storage
  ...
  void _initDeepLinkListener() async {
    try {
      final initialUri =null;// await getInitialUri();
  ...
  // 🔍 National Number Validation Function
  bool _validateNationalNumber(String nationalNumber) {
    ...
    if (firstDigit != '2' && firstDigit != '9') {
      print("❌ National number validation failed: must start with 2 or 9");
      return false;
    }

    print("✅ National number validation passed: $nationalNumber");
    return true;
  }

  Future<void> _trySanadVerification() async {
    print("✅ _trySanadVerification() called");
    setState(() {
      isloading = true;
    ...
    try {
      ...
      print("eKYC Token: ${globalVars.ekycTokenID}");
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (result["data"] != null && result["data"]["verified"] == true) {
            ...
            print("Sanad Redirect URL: $redirectUrl");
            ...
            if (await canLaunchUrl(sanadUrl)) {
              print("✅ Launching Sanad URL...");
              await launchUrl(sanadUrl, mode: LaunchMode.externalApplication);
              print("✅ Sanad URL launched - waiting for callback");
              ...
              // Call PostValidateSubscriber API to get username, password, price ,SHADERMA
          ...
          print("❌ Sanad API returned non-200 status: ${response.statusCode}");
          setState(() {
            globalVars.sanadValidation = true; // ✅ Show toggle
            isloading = false;
            checkNationalDisabled = false;
...
//I/flutter (26370): API Response: {"code":"cdd02bb7-7cb0-4264-8856-6f568f882a25","errorName":"AppException","message":{"ar":"غير مسموح بتغيير حالة الملف","en":"Change status of dossier forbidden"},"method":"POST","path":"/wid-zain/api/v1/session/28327f83-692b-48cf-b5a6-6cce2d28b4a1/sanad/authorize","timestamp":"2025-07-02T23:09:10.244Z","success":false}
        ...
        if(result["success"] == true && result['message']['en'] == "The Operation has been Successfully Completed"){
          if(result['data']!= null ){
            if(result['data']['nationalNo'] != nationalNo){
...
// New method to show wrong side warning
  ...
  Future<void> _initializeCameraMRZ() async {
    ...
    try {
      ...
      print("✅ Loading hidden: _isLoadingPassportCamera = $_isLoadingPassportCamera");
...
// Convert image file to Base64
  ...
  huploadBackID_API(File backImageFile) async {
    setState(() {
      isloading = true;
      print("📤 huploadBackID_API - Loading started: isloading = $isloading");
    });
    final client = createHttpClient();
    try {
      // Create the URL with path parameter
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        setState(() {
          isloading = false;
          print("✅ huploadBackID_API - Success, loading stopped: isloading = $isloading");
          // Set the base64 string for display
  ...
  uploadPassportFile_API(File frontImageFile) async {
    setState(() {
      isloading = true;
      print("📤 uploadPassportFile_API - Loading started: isloading = $isloading");
    });
    final client = createHttpClient();
    try {
      // Create the URL with path parameter
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        setState(() {
          isloading = false;
          print("✅ uploadPassportFile_API - Success, loading stopped: isloading = $isloading");
          // Set the base64 string for display
  ...
  uploadPassportTempFile_API(File frontImageFile) async {
    setState(() {
      isloading = true;
      print("📤 uploadPassportTempFile_API - Loading started: isloading = $isloading");
    });
    final client = createHttpClient();
    try {
      // Create the URL with path parameter
  ...
  uploadForignPassportFile_API(File frontImageFile) async {
    setState(() {
      isloading = true;
      print("📤 uploadForignPassportFile_API - Loading started: isloading = $isloading");
    });
    final client = createHttpClient();
    try {
      // Create the URL with path parameter
  ...
  // 🎯 Barcode API Functions for Foreign Passport
  
  /// Upload barcode as image file
  uploadBarcodeImage_API(File barcodeImageFile) async {
    setState(() {
      isloading = true;
      print("📤 uploadBarcodeImage_API - Loading started: isloading = $isloading");
    });
    final client = createHttpClient();
    try {
      final apiUrl = "https://079.jo/wid-zain/api/v1/barcode/multipart";
      final url = Uri.parse(apiUrl);
      ...
      // Add the barcode image file
      var multipartFile = await http.MultipartFile.fromPath(
        'file', // Field name for image
      ...
      print('Barcode Image Upload - Status Code: ${response.statusCode}');
      print('Barcode Image Upload - Response Body: ${response.body}');

      if (response.statusCode == 200 || response.statusCode == 201) {
        print("✅ Barcode image uploaded successfully");
        setState(() {
          isloading = false;
          print("✅ uploadBarcodeImage_API - Success, loading stopped: isloading = $isloading");
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              EasyLocalization.of(context).locale == Locale("en", "US")
                  ? "Barcode uploaded successfully"
                  : "تم تحميل الباركود بنجاح",
            ),
            backgroundColor: Colors.green,
          ),
        );
        ...
        print('Barcode Upload Error: ${response.statusCode}');
        final Map<String, dynamic> jsonData = jsonDecode(response.body);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              EasyLocalization.of(context).locale == Locale("en", "US")
                  ? jsonData['message']['en'] ?? "Barcode upload failed"
                  : jsonData['message']['ar'] ?? "فشل تحميل الباركود",
            ),
            backgroundColor: Colors.red,
          ),
        );
      ...
      print('Barcode Upload Exception: $e');
  ...
  void documentProcessingID_API() async {
    ...
    // ✅ Show loading indicator - this API takes long time
    setState(() {
      isloading = true;
    });
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          isloading = false;  // ✅ Ensure loading is hidden on error
        });
          ...
          SnackBar(
            content: Text(
              EasyLocalization.of(context).locale == Locale("en", "US")
                  ...
                  : "فشل في معالجة المستند. يرجى المحاولة مرة أخرى.",
            ),
            backgroundColor: Colors.red,
          ),
        );
      ...
      // ✅ Finally block ensures loading is always hidden
      client.close();
    }
  }
  ...
  void documentProcessingForignPassport_API() async {
    ...
    print("🔄 documentProcessingForignPassport_API started - Loading indicator shown");
    ...
    try {
      ...
      if (response.statusCode == 200) {
        print("✅ documentProcessingForignPassport_API - Success response received");
      ...
      if(responseMap['success']==true){
        if(data['status']=="working"){
          ...
          // 🎯 Store passport data for NonJordainianCustomerInformation
          globalVars.fullNameAr="";
          globalVars.fullNameEn=data['ocrData']['givenNames']; // Given names for first/second/third name
          globalVars.surname=data['ocrData']['surname'] ?? ""; // Last name
          globalVars.natinalityNumber=data['nationalNumber'] ?? data['ocrData']['nationalNumber'] ?? "-"; // Passport Number
          globalVars.cardNumber=data['ocrData']['documentNumber']; // Document number
          
          // Store date of birth components
          ...
          globalVars.birthDay=data['ocrData']['dateOfBirth']['day'].toString();
          globalVars.birthMonth=data['ocrData']['dateOfBirth']['month'].toString();
          globalVars.birthYear=data['ocrData']['dateOfBirth']['year'].toString();
          ...
          globalVars.expiryDay=data['ocrData']['expirationDate']['day'].toString();
          globalVars.expiryMonth=data['ocrData']['expirationDate']['month'].toString();
          globalVars.expiryYear=data['ocrData']['expirationDate']['year'].toString();
          
          globalVars.gender=data['ocrData']['sex'];
          globalVars.bloodGroup="-";
          globalVars.registrationNumber="-";
          
          print("📝 Foreign Passport Data Stored:");
          print("  - Given Names: ${globalVars.fullNameEn}");
          print("  - Surname: ${globalVars.surname}");
          print("  - National Number (Passport): ${globalVars.natinalityNumber}");
          print("  - Birth Date: ${globalVars.birthdate}");
          print("  - Expiry Date: ${globalVars.expirayDate}");
          setState(() {
            globalVars.showPersonalNumber=true;
            _LoadImageForeignPassport=true; // ✅ Show foreign passport image
            showBarcodeField = true; // 🎯 Show barcode field after successful passport processing
          });
          print("🎯 Barcode field enabled - waiting for barcode submission");
      ...
      // ✅ Error handling - always hide loading
      print("❌ documentProcessingForignPassport_API - Error occurred: $e");
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              EasyLocalization.of(context).locale == Locale("en", "US")
                  ? "Failed to process foreign passport. Please try again."
                  : "فشل في معالجة جواز السفر الأجنبي. يرجى المحاولة مرة أخرى.",
            ),
            backgroundColor: Colors.red,
          ),
        );
  ...
  void documentProcessingPassport_API() async {
    ...
    print("🔄 documentProcessingPassport_API started - Loading indicator shown");
    ...
    try {
      ...
      if (response.statusCode == 200) {
        print("✅ documentProcessingPassport_API - Success response received");
      ...
      if(responseMap['success']==true){
        if(data['status']=="working"){
          ...
          print("📝 Temp Passport Data Stored:");
          print("  - Given Names: ${globalVars.fullNameEn}");
          print("  - Surname: ${globalVars.surname}");
          print("  - National Number (Passport): ${globalVars.natinalityNumber}");
          print("  - Birth Date: ${globalVars.birthdate}");
      ...
      // ✅ Error handling - always hide loading
      print("❌ documentProcessingPassport_API - Error occurred: $e");
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              EasyLocalization.of(context).locale == Locale("en", "US")
                  ? "Failed to process passport. Please try again."
                  : "فشل في معالجة جواز السفر. يرجى المحاولة مرة أخرى.",
            ),
            backgroundColor: Colors.red,
          ),
        );
  ...
  Widget get msg {
    return BlocBuilder<PostValidateSubscriberBlock, PostValidateSubscriberState>(builder: (context, state) {
      // ✅ Only show loading for BLoC state (NOT isloading - that's handled by global overlay)
      ...
      if (state is PostValidateSubscriberSuccessState) {
        ...
        child:Column(
          ...
          if (globalVars.sanadValidation) ...[
            ...
            // Document Type Selection Toggle (similar to GSM)
            Text(
              EasyLocalization.of(context).locale == Locale("en", "US")
              ...
              children: [
                Expanded(
                  child: ElevatedButton(
                    ...
                    onPressed: _isLoadingIDCamera ? null : () {
                      ...
                      // Use Future.microtask to avoid calling async during build
                    ...
                    child: _isLoadingIDCamera
                        ...
                        : Text(
                            EasyLocalization.of(context).locale == Locale("en", "US")
                                ? (globalVars.capturedBase64.isNotEmpty && globalVars.capturedBase64.length >= 2 ? "Rescan ID" : "Scan ID")
                                : (globalVars.capturedBase64.isNotEmpty && globalVars.capturedBase64.length >= 2 ? "إعادة مسح الهوية" : "مسح الهوية"),
                          ),
                  ),
                ),
                SizedBox(width: 10),
                Expanded(
                  child: ElevatedButton(
                    ...
                    onPressed: _isLoadingPassportCamera ? null : () {
                      print("\u2705 Scan Jordanian Passport button pressed");
                      if (!mounted) return;
                      
                      setState(() {
                        globalVars.tackJordanPassport = true;
                        globalVars.tackID = false;
                        _isLoadingPassportCamera = true; // ✅ Show loading
                        print("🔄 Loading state set: _isLoadingPassportCamera = $_isLoadingPassportCamera");
                      });
                      ...
                      Future.microtask(() async {
                        if (!mounted) return;
                        try {
                          print("\u2705 State updated - opening passport camera...");
                          await _initializeCameraMRZ();
                          print("\u2705 Passport camera initialized successfully");
                        } catch (e) {
                          print("❌ Error initializing passport camera: $e");
                          if (mounted) {
                            setState(() {
                              _isLoadingPassportCamera = false; // ✅ Hide loading on error
                            });
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: Text(
                    ...
                    child: _isLoadingPassportCamera
                        ? Center(
                            child: SizedBox(
                              height: 20,
                              width: 20,
                              child: CircularProgressIndicator(
                                strokeWidth: 2.5,
                                valueColor: AlwaysStoppedAnimation<Color>(
                                  globalVars.tackJordanPassport ? Colors.white : Color(0xFF4f2565),
                                ),
                              ),
                            ),
                          )
                        : Text(
                            EasyLocalization.of(context).locale == Locale("en", "US")
                                ...
                                : (_loadImageJordanianPassport || JordanianPassport.isNotEmpty ? "إعادة مسح جواز" : "جواز سفر أردني"),
                          ),
                  ),
                ),
              ],
    ...
    return BlocListener<PostValidateSubscriberBlock, PostValidateSubscriberState>(
      ...
      child: Column(
        ...
        children: <Widget>[
            ...
            children: [
              Expanded(
                child: ElevatedButton(
                  ...
                  onPressed: _isLoadingTemporaryCamera ? null : () async {
                    setState(() {
                      globalVars.tackTemporary = true;
                      globalVars.tackForeign = false;
                      _isLoadingTemporaryCamera = true; // ✅ Show loading
                    });
                    ...
                    try {
                      ...
                      if (isEkycInitialized) {
                        await _initializeCameraTemporary(); // Open Temporary Passport camera
                      } else {
                        setState(() {
                          _isLoadingTemporaryCamera = false; // ✅ Hide loading on error
                        });
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                  ...
                  child: _isLoadingTemporaryCamera
                      ? Center(
                          child: SizedBox(
                            height: 20,
                            width: 20,
                            child: CircularProgressIndicator(
                              strokeWidth: 2.5,
                              valueColor: AlwaysStoppedAnimation<Color>(
                                globalVars.tackTemporary ? Colors.white : Color(0xFF4f2565),
                              ),
                            ),
                          ),
                        )
                      : Text(
                          EasyLocalization.of(context).locale == Locale("en", "US")
                              ? (_LoadImageTemporaryPassport || TemporaryPassport.isNotEmpty ? "Rescan Temporary" : "Temporary Passport")
                              : (_LoadImageTemporaryPassport || TemporaryPassport.isNotEmpty ? "إعادة مسح مؤقت" : "جواز سفر مؤقت"),
                        ),
                ),
              ),
              SizedBox(width: 10),
              Expanded(
                child: ElevatedButton(
                  ...
                  onPressed: _isLoadingForeignCamera ? null : () async {
                    setState(() {
                      globalVars.tackForeign = true;
                      globalVars.tackTemporary = false;
                      _isLoadingForeignCamera = true; // ✅ Show loading
                    });
                    ...
                    try {
                      ...
                      if (isEkycInitialized) {
                        await _initializeCameraForeign(); // Open Foreign Passport camera
                      } else {
                        setState(() {
                          _isLoadingForeignCamera = false; // ✅ Hide loading on error
                        });
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                  ...
                  child: _isLoadingForeignCamera
                      ? Center(
                          child: SizedBox(
                            ...
                            child: CircularProgressIndicator(
                              strokeWidth: 2.5,
                              valueColor: AlwaysStoppedAnimation<Color>(
                                globalVars.tackForeign ? Colors.white : Color(0xFF4f2565),
                              ),
                            ),
                          ),
                        )
                      : Text(
                          EasyLocalization.of(context).locale == Locale("en", "US")
                              ? (_LoadImageForeignPassport || ForeignPassport.isNotEmpty ? "Rescan Foreign" : "Foreign Passport")
                              : (_LoadImageForeignPassport || ForeignPassport.isNotEmpty ? "إعادة مسح أجنبي" : "جواز سفر أجنبي"),
                        ),
                ),
              ),
            ],
          ...
          // 🎯 Show Next button when document processing successful
          // Barcode will be submitted when Next is clicked for Foreign passport
          if (_documentProcessingSuccess) ...[
            ...
            Container(
              ...
              child: TextButton(
                onPressed: () async {
                  print("✅ Next button pressed - processing...");
                  
                  // 🎯 For Foreign passport, check if barcode was submitted
                  if (globalVars.tackForeign) {
                    String barcodeValue = barcodeController.text.trim();
                    
                    // Check if barcode is present and submitted
                    if (barcodeValue.isEmpty) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            EasyLocalization.of(context).locale == Locale("en", "US")
                                ? "Please enter or scan the barcode before proceeding."
                                : "يرجى إدخال أو مسح الباركود قبل المتابعة.",
                          ),
                          backgroundColor: Colors.orange,
                        ),
                      );
                    ...
                    // If barcode not yet uploaded (manual entry case), submit it now
                    if (!barcodeUploaded) {
                      print("📝 Submitting manually entered barcode: $barcodeValue");
                      
                      // Validate first
                      bool isValid = _validateBarcode(barcodeValue);
                      if (!isValid) {
                        return; // Stop execution
                      }
                      passportNo.text=barcodeValue;
                      // Submit to API using existing function
                      await uploadBarcodeManual_API(barcodeValue);
                      
                      // Check if upload was successful (uploadBarcodeManual_API sets barcodeUploaded flag)
                      if (!barcodeUploaded) {
                        // API failed (uploadBarcodeManual_API already showed error message)
                        return; // Stop execution
                      }
                    }
                    
                    print("✅ Barcode already submitted, proceeding to PostValidateSubscriber...");
                  }
                  
                  // Call PostValidateSubscriber API for non-Jordanian
